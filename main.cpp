  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
 		  ...
 		  /* The desired steering angle is from the current position of the car (in x_position, y_position) and the desired position (at the end of x_point[], y_point[]. The actual steering angle is given by yaw, so the error is the difference between the desired and actual steering angles */
